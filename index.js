if(window.innerWidth < 1200) {
    document.querySelector('.complect__wrapper').classList.add('owl-carousel');
    $(document).ready(function(){
        $(".complect__wrapper").owlCarousel({
            items: 1    
        });
    });
}
...
const timeCount = () => {
    ...
    progressDaysDesk.setAttribute(
        "stroke-dasharray",
        `${0 + daysV * 3.333}  100`
    );
    progressHoursDesk.setAttribute(
        "stroke-dasharray",
        `${0 + hoursV * 4.166}  100`
    );
    progressMinutesDesk.setAttribute(
        "stroke-dasharray",
        `${0 + minutesV * 1.667}  100`
    );
    progressSecondsDesk.setAttribute(
        "stroke-dasharray",
        `${0 + secondsV * 1.667}  100`
    );
};
timeCount();
